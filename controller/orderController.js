...
const orderDetailes = async (req, res) => {
  try {
    ...
    const orderData = await OrderDB.findOne({ _id })
      .populate("userId")
      .populate("products.productId");
...
const adminReturnOrder = async (req, res) => {
  try {
    ...
    const orderData = await OrderDB.findById({ _id: orderId })
      .populate("userId")
      .populate("products.productId");
    if (!orderData) {
      return res
        .status(200)
        .json({ success: false, message: "Order Not Fount!." });
    }
    ...
    if (status == "return") {
      await ProductDb.findByIdAndUpdate(
        ...
        { $inc: { stock: product.quandity, orderCount: -product.quandity } }
      );
      await WalletDB.findOneAndUpdate(
        { userId: orderData.userId },
        {
          $inc: { Balance: product.productTotal },
          $push: {
            walletHistery: `Credit : ${product.productId.name} refunded amount of Rs.${product.productTotal}.00`,
          },
        }
      );
    }
    ...
    const total = orderData.products.reduce((acc, cur) => {
      if (
        ...
        cur.productStatus == "Delivered" ||
        cur.productStatus == "returnPending"
      ) {
...
const orderStatus = async (req, res) => {
  try {
    ...
    const orderData = await OrderDB.findOne({ _id }).populate([
      {
    ...
    switch (changeStatus) {
      case "canceled":
        for (let i = 0; i < orderData.products.length; i++) {
          if (orderData.products[i].productStatus == "pending") {
            check = true;
            orderData.products[i].productStatus = changeStatus;
            if (orderData.products[i].paymentMethod !== "COD") {
              orderData.products[i].productId.stock =
                orderData.products[i].productId.stock +
                orderData.products[i].quandity;
              await orderData.products[i].productId.save();
              await WalletDB.findOneAndUpdate(
                { userId: orderData.userId },
                {
                  $inc: {
                    Balance:
                      orderData.products[i].productTotal -
                      Math.floor(
                        orderData.couponOfferPrice / orderData.products.length
                      ),
                  },
                  $push: {
                    walletHistery: `${new Date().toLocaleDateString()} : Credit : ${
        ...
        if (!check) {
          return res.json({
            success: false,
            message: "You can only cancell the pending orders!.",
          });
        }
      ...
      case "Delivered":
        for (let i = 0; i < orderData.products.length; i++) {
          if (orderData.products[i].productStatus == "pending") {
            check = true;
            orderData.products[i].productStatus = changeStatus;
            orderData.products[i].productId.categoryID.orderCount =
              orderData.products[i].productId.categoryID.orderCount +
              orderData.products[i].quandity;
            orderData.products[i].productId.orderCount =
              orderData.products[i].productId.orderCount +
              orderData.products[i].quandity;
            await orderData.products[i].productId.categoryID.save();
          }
        }
        ...
        if (!check) {
          return res.json({
            success: false,
            message: "You can only Delivered the pending orders!.",
          });
        }
        break;
    }
    ...
    return res.json({
      success: true,
      message: "Successfully change the order status!.",
      orderData,
      total
    });
  } catch (error) {
...
// const singleOrderStatus = async (req, res) => {
//   const { _id, productId } = req.query;
//   const { status } = req.body;
//   const orderData = await OrderDB.findById({ _id }).populate([
//     { path: "userId" },
//     {
//       path: "products.productId",
//       populate: { path: "categoryID" },
...
//     return res.json({ success: false, message: "Order Not Found!." });
//   }
//   const product = orderData.products.find((order) => order._id == productId);
//   if (!product) {
//     return res.json({ success: false, message: "Order Not Found!." });
//   }
...
//     return res.json({
//       success: false,
//       message: "You cannot change the status of Delivered Product!.",
//     });
//   }
//   if (product.productStatus == "canceled") {
//     return res.json({
//       success: false,
//       message: "You cannot change the status of Cancelled Product!.",
//     });
//   }
...
//       break;
//     case "Delivered":
//       product.productId.categoryID.orderCount =
//         product.productId.categoryID.orderCount + product.quandity;
//       product.productId.orderCount =
//         product.productId.orderCount + product.quandity;
//       await product.productId.categoryID.save();
//       break;
//   }
//   const total = orderData.products.reduce((acc, cur) => {
//     if (
...
//     message: "Successfully change the order status!.",
//     total,
//     grandTotal: total - orderData.couponOfferPrice,
//   });
// };
    ...
    switch (sort) {
      case "Name Z - A":
        orders = orders.sort((a, b) =>
          b.userId.name.toLowerCase().localeCompare(a.userId.name.toLowerCase())
        );
        break;
      case "Name A - Z":
        orders = orders.sort((a, b) =>
          a.userId.name.toLowerCase().localeCompare(b.userId.name.toLowerCase())
        );
        break;
      case "Old":
        orders = orders.sort(
...
const searchOrders = async (req, res) => {
  try {
    ...
    orders = orders.filter((product) =>
      product.userId.name.toLowerCase().includes(search.toLowerCase())
    );
    const totalPage = Math.ceil(orders.length / limit);
    orders = orders.slice(0, limit);
...
const order = async (req, res) => {
  try {
    const { cartData, wishlistData, cartTotal } = await getStoreDataForUser(
      req,
      res
    );
    const limit = 4;
    let data = await OrderDB.find({ userId: req.session.user_id })
      .populate("userId")
      .populate("products.productId")
      .sort({ createdAt: -1 });
...
const placeOrder = async (req, res) => {
  try {
    ...
    const cherckStock = cartData.products.filter(
      (value) =>
        value.productId.stock == 0 || value.productId.stock < value.quandity
    );
    ...
    let money = cartData.products.reduce((acc, value) => {
      if (value.productId.stock > 0) {
        const offerProduct = offerData.find(
          (offer) =>
            offer.iteam === value.productId.name ||
            offer.iteam === value.productId.categoryID.name
        );
        if (offerProduct) {
          return (
            acc +
            value.productId.Price * value.quandity -
            ((parseInt(value.productId.Price) * offerProduct.offerRate) / 100) *
              value.quandity
          );
        } else {
...
const saveOrder = async (req, res) => {
  try {
    ...
    const products = await CartDB.findOne({ userId: req.session.user_id })
      .populate("userId")
      .populate("products.productId")
      .populate("products.productId.categoryID");
    ...
    productsData.filter((product) => {
      if (product.productId.stock < product.quandity) {
        return res.json({
          success: false,
          message: "Some of your product from your Cart is out of stock!.",
        });
      }
    });

    const offerData = await OfferDB.find({});
    const addressdb = await AddressDB.findOne({
      userID: req.session.user_id,
    }).populate("userID");
    ...
    for (let i = 0; i < productsData.length; i++) {
      let product = await ProductDb.findOne({
        _id: productsData[i].productId._id,
      });
      let category = await CategoryDb.findById({
        _id: productsData[i].productId.categoryID._id,
      });
      ...
      let money = 0;
      const offerProduct = offerData.find(
        (offer) => offer.iteam === product.name || offer.iteam === category.name
      );

      if (offerProduct) {
        (money =
          product.Price * productsData[i].quandity -
          Math.floor(
            (product.Price *
              productsData[i].quandity *
              offerProduct.offerRate) /
              100
          )),
          "money",
          product.Price * productsData[i].quandity - Math.floor(product.Price);
        (calculatingMoney +=
          product.Price * productsData[i].quandity -
          Math.floor(
            (product.Price *
...
const payAgain = async (req, res) => {
  try {
    ...
    if (product.productId.stock < quandity) {
      return res.json({
        success: false,
        message: "Product Quandity Exceeded The Product Stock!.",
      });
    }
...
const payAgainAll = async (req, res) => {
  try {
    ...
    for (let i = 0; i < orderData.products.length; i++) {
      ...
      if (
        orderData.products[i].productId.stock < orderData.products[i].quandity
      ) {
        return res.json({
          success: false,
          message: "Product Quandity Exceeded The Product Stock!.",
...
const returnOrder = async (req, res) => {
  try {
    ...
    console.log(productData.createdAt,date)
    const date2 = productData.createdAt;

    const diffTime = Math.abs(date - date2); 
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
    if(diffDays <= 7){
       return res.json({success:false,message:"you can only return a product that is ordered 10 days before"});
    }
    ...
    return res.json({
      success: true,
      message:
        "Product Return Request Sented Successfully. Meney Will Credit to Your Wallet After Admin Accept your Return Request",
    });
  } catch (error) {
    console.log(error.message);
  }
...
const returnAllOrder = async (req, res) => {
  try {
    ...
    const productData = await OrderDB.findById({ _id }).populate(
      "products.productId"
    );
    if (!productData) {
      return res.json({ success: false, message: "Order Not Found!." });
...
const orderUserDetailes = async (req, res) => {
  try {
    ...
    const { _id } = req.query;
    const orders = await OrderDB.findOne({ _id: _id })
      .populate("userId")
      .populate("products.productId");

    return res.render("orderDetailes", {
      orderData: orders,
      cartData,
      wishlistData,
      cartTotal,
...
const cancelUserOrder = async (req, res) => {
  try {
    ...
    const orderData = await OrderDB.findOne({ _id: _id })
      .populate("userId")
      .populate("products.productId");
        ...
        { userId: req.session.user_id },
        {
          ...
          $push: {
            walletHistery: `${new Date().toLocaleDateString()} : Credit : ${
              ...
              order.productTotal -
              Math.floor(orderData.couponOfferPrice / orderData.products.length)
            }.00`,
          },
        },
        { new: true }
    ...
    return res.send({
      success: true,
      message: "Successfully Cancelled this Products!.",
      clearAction,
      total,
      grandTotal: total - orderData.couponOfferPrice,
    });
...
const cancelAllProductInOrder = async (req, res) => {
  try {
    ...
    for (let product of products) {
      const productCheck = await ProductDb.findById({
        _id: product.productId._id,
      });
      if (!productCheck) {
        return res.json({ success: false, message: "Product Not Found!." });
    ...
    res.send({
      success: true,
      message: "Successfully Cancelled All Products!.",
      orderData,
    });
  } catch (error) {
    console.log(error.message);
